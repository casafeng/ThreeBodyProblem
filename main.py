...
inital_position_1 =  [1.0, 0.0,  0.0]
inital_position_2 =  [-0.5, np.sqrt(3)/2,  0.0]
inital_position_3 =  [-0.5, -np.sqrt(3)/2, 0.0]

# Velocity
omega = 0.8
inital_velocity_1 =  [0,  omega, 0]
inital_velocity_2 =  [0.7*omega, -0.35*omega, 0.0]
inital_velocity_3 =  [-0.7*omega, -0.35*omega, 0.0]
...
animation = FuncAnimation(fig, update, frames=range(0, len(t_points), 5), interval=20)
animation.save("3body.gif", writer="pillow", fps=30)
plt.show()
...
line1, = ax2.plot([], [], [], 'green', linewidth=1)
dot1, = ax2.plot([], [], [], 'o', color='green', markersize=5)

line2, = ax2.plot([], [], [], 'red', linewidth=1)
dot2, = ax2.plot([], [], [], 'o', color='red', markersize=5)

line3, = ax2.plot([], [], [], 'blue', linewidth=1)
dot3, = ax2.plot([], [], [], 'o', color='blue', markersize=5)

ax2.set_xlim(np.min(p1x_sol)-1, np.max(p1x_sol)+1)
ax2.set_ylim(np.min(p1y_sol)-1, np.max(p1y_sol)+1)
ax2.set_zlim(np.min(p1z_sol)-1, np.max(p1z_sol)+1)
ax2.set_title("3-Body Simulation (GIF)")
ax2.set_xlabel("x"); ax2.set_ylabel("y"); ax2.set_zlabel("z")

def update_save(frame):
    line1.set_data(p1x_sol[:frame], p1y_sol[:frame])
    line1.set_3d_properties(p1z_sol[:frame])
    dot1.set_data([p1x_sol[frame]], [p1y_sol[frame]])
    dot1.set_3d_properties([p1z_sol[frame]])

    line2.set_data(p2x_sol[:frame], p2y_sol[:frame])
    line2.set_3d_properties(p2z_sol[:frame])
    dot2.set_data([p2x_sol[frame]], [p2y_sol[frame]])
    dot2.set_3d_properties([p2z_sol[frame]])

    line3.set_data(p3x_sol[:frame], p3y_sol[:frame])
    line3.set_3d_properties(p3z_sol[:frame])
    dot3.set_data([p3x_sol[frame]], [p3y_sol[frame]])
    dot3.set_3d_properties([p3z_sol[frame]])
...
anim2.save("three_body.gif", writer="pillow", fps=30)
print("GIF saved as: three_body.gif")
